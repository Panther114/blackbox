...
export function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setConfig(previous => ({ ...previous, username: String(cfg.username || ''), password: loadedPassword, downloadDir: String(cfg.downloadDir || './downloads'), headless: Boolean(cfg.headless ?? true), autoCheckUpdates: Boolean(cfg.autoCheckUpdates ?? true) }));
  ...
  async function saveSettings(testLogin: boolean) {
    await runWithUiError(async () => {
      const passwordToDisplay = config.password || savedPassword;
      setStatus(testLogin ? 'Saving settings and testing login...' : 'Saving settings...'); if (DEMO_MODE || !window.blackboxGui) await delay(350); else await window.blackboxGui.saveSetup({ ...config, testLogin }); setSavedPassword(passwordToDisplay); setConfig(previous => ({ ...previous, password: passwordToDisplay })); setHasCredentials(Boolean(config.username.trim()) && Boolean(passwordToDisplay)); setStatus(testLogin ? 'Settings saved. Login test requested.' : 'Settings saved.');
    });
  }

  async function resetCredentials() {
    await runWithUiError(async () => {
      if (DEMO_MODE || !window.blackboxGui) await delay(250); else await window.blackboxGui.resetSetup(); setHasCredentials(false); setSavedPassword(''); setConfig(previous => ({ ...previous, username: '', password: '' })); setShowPassword(false); setStatus('Credentials reset.');
    });
  }
